...
public class vThirdPersonCamera : NetworkBehaviour
{
    ...
    public bool enable_debug_cursor_unlock = false;
    public GameObject Mobile_controls;
    public AimAssist aim_assist;
    public StarterAssetsInputs newinputs;
    public bool aim_assist_enabled = false;
...
aim_assist_enabled = true;
    ...
    {
        var rotationChanges = aim_assist.TrackTarget();

    // add to horizontal rotation
    if(newinputs.shoot || newinputs.aim)
    {
    mouseX += rotationChanges.TurnAddition.y;

    // add to vertical rotation
    mouseY += rotationChanges.PitchAdditionInDegrees;
    }
    }
