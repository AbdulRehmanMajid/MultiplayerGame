...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : MonoBehaviour
    {
        ...
        public float sprintSpeed_new = 18f;
        public float default_speed = 8f;
        public float gun_use_speed = 6f;
        ...
        public virtual void SetControllerMoveSpeed(vMovementSpeed speed)
        {
            ...
            else if(!speed.walkByDefault && !isaim)
                moveSpeed = Mathf.Lerp(moveSpeed, isSprinting ? sprintSpeed_new : default_speed, speed.movementSmooth * Time.deltaTime);
                else if (isaim)
                {
                     moveSpeed = Mathf.Lerp(moveSpeed, gun_use_speed, speed.movementSmooth * Time.deltaTime);
        ...
        public virtual void RotateToPosition_aim(Vector3 position)
        {
            
            Vector3 desiredDirection = position - transform.position;
            RotateToDirection_aim(desiredDirection.normalized);
        }

        public virtual void RotateToDirection_aim(Vector3 direction)
        {
            
            RotateToDirection_aim(direction, isStrafing ? strafeSpeed.rotationSpeed : freeSpeed.rotationSpeed);
        }

        public virtual void RotateToDirection_aim(Vector3 direction, float rotationSpeed)
        {
